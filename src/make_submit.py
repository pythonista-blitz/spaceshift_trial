...
def make_json(annotations_dir, categories):
    ...
    for annotation_file in annotation_files:
        ...
        img = cv2.imread(os.path.join(
            annotations_dir, annotation_file), flags=cv2.IMREAD_GRAYSCALE)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--annotations_dir', type=str,
                        help='directory of the annotations', nargs='?')
    ...
    with open(f'../submit/{date.strftime("%Y%m%d")}_黒瀬一平_submit.json', 'w') as f:
